...
impl Loader<(Uuid, i32), UniverseVersion> {
    ...
    pub async fn load_current(&self, universe_id: Uuid, unreleased: bool) -> anyhow::Result<Option<UniverseVersion>> {
        let version: Option<UniverseVersion> = tokio::task::block_in_place(|| -> anyhow::Result<Option<UniverseVersion>> {
